...
class SpawTbItem
{
   ...
   * @see SPAW_AGENT_UNSUPPORTED, SPAW_AGENT_IE, SPAW_AGENT_GECKO, SPAW_AGENT_ALL
   */
  public function __construct($module, $name, $agent=SPAW_AGENT_ALL)
  {
    $this->module = $module;
    $this->name = $name;
    $this->agent = $agent;
...
class SpawToolbar
{
       ...
       while (($pg = readdir($dh)) !== false) {
        if (file_exists($pgdir.$pg.'/lib/toolbars/'.$this->name.'.toolbar.php')) {
          ...
          /*if ($pgdir[0]=== '/') {
            ...
            include($pgdir1.$pg.'/lib/toolbars/'.$this->name.'.toolbar.php');
          } else {*/
  ...
  function renderToolbar($prefix, $theme) {
    ...
    $pgdir = SpawConfig::getStaticConfigValue("SPAW_ROOT").'plugins/';
    # ROMANS CODE
    $pgdir='mods/spaw2full/plugins/';

    # This does not work
    # ROMANS CODE
    # ПРОВЕРИТЬ items
    #if ($pgdir[0]=== '/') {
    #  $pgdir = substr($pgdir,1);
    #}
    #print "I am here!";
    $i = 0;
    if ($this->items) {
