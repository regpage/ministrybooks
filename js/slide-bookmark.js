...
return str;}
function getRootWindow(){var win=window;while(win!=undefined){try{if(win===win.parent){break;}else if(win.parent!=undefined&&win.parent.document.location.href.indexOf("selenium-server/default.htm")!=-1){break;}
...
this.visible=false;};var MIN_THUMBNAIL_DIM=40;function getParentLink(node){if(node.nodeName.toLowerCase()=="a"){return node.cloneNode(true);}
if(node.parentNode==undefined||node.parentNode===node)
return undefined;else
return getParentLink(node.parentNode);}
function linkPhotos(viewer,node){var i;if(node==undefined)
return;if(isThumbnail(node)){var link=getParentLink(node);if(findImageLink(link,node.src)){var photo=new PhotoImg(undefined,link.href);var found=false;for(i=0;i<viewer.photos.length;i++){if(viewer.photos[i].src==photo.src){found=true;break;}}
if(!found){viewer.add(photo);}}}
if(node.childNodes!=undefined){for(i=0;i<node.childNodes.length;i++){linkPhotos(viewer,node.childNodes[i]);}}}
function isThumbnail(node){if(node.nodeName.toLowerCase()!="img")
return false;if(!isNaN(node.width)&&node.width<MIN_THUMBNAIL_DIM)
return false;if(!isNaN(node.height)&&node.height<MIN_THUMBNAIL_DIM)
return false;return true;}
var IMAGE_FORMATS=["jpg","jpeg","png","gif","bmp","tif","tiff"];function findImageLink(link,src){if(link==undefined||link.href==undefined)
return false;var i;for(i=0;i<IMAGE_FORMATS.length;i++){if(link.href.indexOf("."+IMAGE_FORMATS[i])!=-1){return true;}}
var flickrRE=/.+static\.flickr\.com.+_.\.jpg/;if(flickrRE.test(src)){link.href=src.replace(/_.\.jpg/,"_b.jpg");return true;}
return false;}
function initDefaultViewer(){var i;if(defaultViewer==undefined){if(document.viewer==undefined){defaultViewer=new PhotoViewer();}else{defaultViewer=document.viewer;defaultViewer.photos=[];defaultViewer.index=0;}}else{defaultViewer.photos=[];defaultViewer.index=0;}
linkPhotos(defaultViewer,window.document.body);if(defaultViewer.photos.length>0){defaultViewer.show();}else{defaultViewer=undefined;}}
initDefaultViewer();